...
class KeyDatabase(object):
    ...
    """
    def __init__(self, secret="", keyprefix="STM"):
        self.secret = secret
    ...
    def load_from_file(self, filename):
        with open(filename, "r") as f:
            db = json.load(f)
        if (db["secret"] != self.secret) or (
            db["keyprefix"] != self.keyprefix):
            raise RuntimeError("Couldn't load keydb")
        self.seed2pair.update(db["seed2pair"])
        return

    def save_to_file(self, filename):
        d = {"secret" : self.secret, "keyprefix" : self.keyprefix, "seed2pair" : self.seed2pair}
        with open(filename, "w") as f:
            json.dump(d, f, separators=(",", ":"), sort_keys=True)
        return

    def get(self, name, role="active"):
        seed = self.secret+"-"+role+"-"+name
        pair = self.seed2pair.get(seed)
        if pair is None:
            priv = get_key_from_seed(seed)
...
def build_actions(conf):
    keydb = KeyDatabase(secret=conf["secret"], keyprefix=conf["keyprefix"])
    keydb_file = conf.get("keydb_file")
    if keydb_file and os.path.exists(keydb_file):
        keydb.load_from_file(keydb_file)
    ...
    for b in batch(build_setup_transactions(conf, keydb), conf["transactions_per_block"]):
        yield ["wait_blocks", {"count" : 1}]
        for tx in b:
            yield ["submit_transaction", {"tx" : tx}]
        if keydb_file:
            keydb.save_to_file(keydb_file)
...
def main(argv):
    parser = argparse.ArgumentParser(description="Generate transactions for Steem testnet")
    ...
    for tx in build_actions(conf):
        outfile.write(json.dumps(tx, separators=(",", ":")))
        outfile.write("\n")
